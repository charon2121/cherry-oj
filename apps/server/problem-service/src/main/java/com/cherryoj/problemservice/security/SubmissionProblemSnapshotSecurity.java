...
class SubmissionProblemSnapshotSecurity {
    @Bean @Order(-10)
    SecurityFilterChain submissionProblemSnapshotChain(HttpSecurity http,
            @Value("${cherry.service-calls.submission-problem-tokens:}") String tokens) throws Exception {
        http.securityMatcher("/internal/submission/problem-snapshot")
