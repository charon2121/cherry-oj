...
func TestClientStreamsInputsAndOutputs(t *testing.T) {
	request := testRequest()
	request.Inputs = []hostexec.Input{{Path: "main.cpp", SizeBytes: 3}}
	request.StdinBytes = 2
	request.Outputs = []string{"main"}
	socket := fakeServer(t, func(c net.Conn) {
		var r hostexec.Request
		if err := hostexec.ReadFrame(c, &r, hostexec.MaxFrameBytes); err != nil {
			t.Error(err)
			return
		}
		...
		if err := hostexec.WriteFrame(c, hostexec.Result{Version: 1, Outputs: []hostexec.Output{{Path: "main", SizeBytes: 2}}}, 4<<20); err != nil {
			t.Error(err)
			return
		}
		...
		if err := hostexec.WriteFrame(c, hostexec.Completion{Version: 1, Complete: true}, 1024); err != nil {
			t.Error(err)
		}
	})
	var got string
	_, err := Call(context.Background(), socket, request, io.NopCloser(strings.NewReader("abcde")), func(o hostexec.Output, r io.Reader) error { b, e := io.ReadAll(r); got = string(b); return e })
	if err != nil || got != "ok" {
...
func TestClientEarlyFailureClosesBlockedInput(t *testing.T) {
	socket := fakeServer(t, func(c net.Conn) {
		...
		if err := hostexec.WriteFrame(c, hostexec.Result{Version: 1, Reason: "platform", Error: "startup"}, 4<<20); err != nil {
			t.Error(err)
		}
		if err := hostexec.WriteFrame(c, hostexec.Completion{Version: 1, Complete: true}, 1024); err != nil {
			t.Error(err)
...
func TestClientRejectsUnrequestedArtifact(t *testing.T) {
	socket := fakeServer(t, func(c net.Conn) {
		...
		if err := hostexec.WriteFrame(c, hostexec.Result{Version: 1, Outputs: []hostexec.Output{{Path: "secret", SizeBytes: 1}}}, 4<<20); err != nil {
			t.Error(err)
		}
	})
...
func TestClientRejectsMissingCleanupAcknowledgement(t *testing.T) {
	socket := fakeServer(t, func(c net.Conn) {
		...
		if err := hostexec.WriteFrame(c, hostexec.Result{Version: 1}, 4<<20); err != nil {
			t.Error(err)
		}
	})
...
func TestClientRejectsInvalidCompletion(t *testing.T) {
	for _, completion := range []hostexec.Completion{{Version: hostexec.Version}, {Version: hostexec.Version + 1, Complete: true}} {
		t.Run(fmt.Sprintf("%+v", completion), func(t *testing.T) {
			socket := fakeServer(t, func(c net.Conn) {
				...
				if err := hostexec.WriteFrame(c, completion, 1024); err != nil {
					t.Error(err)
				}
			})
			if _, err := Call(context.Background(), socket, testRequest(), io.NopCloser(strings.NewReader("")), nil); err == nil {
