...
func RunExecStage(s ExecSpec) {
	...
	if err := s.validate(); err != nil {
		terminalFailure(s.ErrorFD, execFailureConfig, unix.EINVAL)
	}
	...
	for resource, limit := range map[int]uint64{syscall.RLIMIT_CORE: 0, syscall.RLIMIT_NOFILE: s.NoFile, syscall.RLIMIT_FSIZE: s.FileSizeBytes} {
		if err := syscall.Setrlimit(resource, &syscall.Rlimit{Cur: limit, Max: limit}); err != nil {
			terminalFailure(s.ErrorFD, execFailureRlimit, failureErrno(err))
		}
	}
	// 标记所有非标准 FD 为 CLOEXEC，不提前关闭 Go runtime 正在使用的 FD。
	// close_range(CLOEXEC) 不允许不可信代码执行前的任何旧根、IPC、memfd 句柄跨 exec。
	if err := unix.CloseRange(ExtraFilesBaseFD, ^uint(0), unix.CLOSE_RANGE_CLOEXEC); err != nil {
		terminalFailure(s.ErrorFD, execFailureCloseOnExec, failureErrno(err))
	}
	if err := dropPrivileges(s.UID, s.GID); err != 0 {
		terminalFailure(s.ErrorFD, execFailurePrivileges, err)
	}
	if err := policy.Install(s.Profile); err != nil {
		terminalFailure(s.ErrorFD, execFailureSeccomp, failureErrno(err))
	}
	if s.ReadyFD != 0 {
		ready := [handshakeBytes]byte{PayloadReady}
		n, _, e := unix.RawSyscall(unix.SYS_WRITE, uintptr(s.ReadyFD), uintptr(unsafe.Pointer(&ready[0])), handshakeBytes)
		if e != 0 || n != handshakeBytes {
			terminalFailure(s.ErrorFD, execFailureHandshake, unix.EPIPE)
		}
		n, _, e = unix.RawSyscall(unix.SYS_READ, uintptr(s.ReadyFD), uintptr(unsafe.Pointer(&ready[0])), handshakeBytes)
		if e != 0 || n != handshakeBytes || ready[0] != PayloadGo {
			terminalFailure(s.ErrorFD, execFailureHandshake, unix.EPIPE)
		}
	...
	terminalFailure(s.ErrorFD, execFailureExecve, execErr)
}
...
func terminalFailure(fd int, stage execFailureStage, errno syscall.Errno) {
	// 直接拼装小端字节；过滤器安装后不引入编码器或新的普通 Go 调用。
	record := [execFailureRecordBytes]byte{
		execFailureStageOffset: byte(stage),
		execFailureErrnoOffset: byte(errno), byte(errno >> 8), byte(errno >> 16), byte(errno >> 24),
	}
	if fd >= ExtraFilesBaseFD {
		_, _, _ = unix.RawSyscall(unix.SYS_WRITE, uintptr(fd), uintptr(unsafe.Pointer(&record[0])), uintptr(len(record)))
	}
	for {
		_, _, _ = unix.RawSyscall(unix.SYS_EXIT_GROUP, launcherFailureExitCode, 0, 0)
	}
}
