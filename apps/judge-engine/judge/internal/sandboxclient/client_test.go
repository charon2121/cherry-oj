...
func TestUpload(t *testing.T) {
	...
	c := sandboxclient.New(srv.URL+"/", time.Second) // 末尾斜杠不能变成 //blobs
...
func TestUploadRejectsNilBody(t *testing.T) {
	c := sandboxclient.New("http://127.0.0.1", time.Second)
...
func TestUploadRequiresRef(t *testing.T) {
	...
	c := sandboxclient.New(srv.URL, time.Second)
...
func TestClientTimeout(t *testing.T) {
	...
	c := sandboxclient.New(srv.URL, 20*time.Millisecond)
	started := time.Now()
	_, err := c.Run(context.Background(), contract.RunSpec{Command: []string{"true"}})
