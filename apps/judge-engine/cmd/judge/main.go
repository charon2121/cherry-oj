...
// 本文件只做参数解析与进程级装配；服务本身在 judge.Run。
func run() int {
	bootstrap := logging.Console("judge", os.Stderr)
	...
	logger, logFiles, err := logging.New("judge", cfg.Logging.Path, cfg.Logging.Level)
	if err != nil {
	...
	defer stop()
	if err := judge.Run(ctx, cfg, logger); err != nil {
		return 1
	}
	return 0
}
