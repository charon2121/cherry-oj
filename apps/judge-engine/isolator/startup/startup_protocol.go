...
// 同一个二进制重新启动自己来扮演 init（P4）与 exec（P5）；main 在做任何其他事之前按这两个参数分流。
const (
	InitArg = "--isolated-init"
	ExecArg = "--isolated-exec"
)
...
// P5 的 terminalFailure 写、P4 的 reportExit 读：stage 占第 0 字节，1～3 保留为零，
...
// ExecSpec 仅由受信 init 生成，路径必须已在独立 rootfs 中解析。
