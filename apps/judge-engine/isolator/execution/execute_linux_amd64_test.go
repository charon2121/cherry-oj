...
func TestFailedStartAlwaysReclaimsOwnedResources(t *testing.T) {
	for _, failure := range []string{"start", "stop", "close"} {
		t.Run(failure, func(t *testing.T) {
			...
			defer cancel()
			cancel()
			result, fatal := newExecution(testRequest(), Options{
				Environment: Environment{StateDir: state, Executable: "/nonexistent-cherry-test-executable"}, Source: strings.NewReader(""), CancelInput: cancel,
				Groups: func(cgroup.Limits) (Group, error) { return g, nil },
			}).Run(ctx)
			defer result.Close()
