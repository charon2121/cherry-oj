...
func TestExecStageFailures(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			defer configR.Close()
			defer configW.Close()
			ready, childReady, e := startup.SocketPair()
			require(t, e)
			defer ready.Close()
			...
			spec := startup.ExecSpec{Path: filepath.Join(base, "does-not-exist"), Args: []string{"never-execute"}, UID: 61002, GID: 61002, NoFile: 256, FileSizeBytes: 64 << 20, Profile: seccomp.Command, ErrorFD: 5}
