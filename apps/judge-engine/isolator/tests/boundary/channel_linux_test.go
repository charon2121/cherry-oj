...
func TestControlSignalChild(t *testing.T) {
	...
	fixture(t)
	control, peer, err := startup.SocketPair()
	require(t, err)
	defer control.Close()
	defer peer.Close()
	...
	_, extra, err := startup.ReceiveEvent(control)
	if extra != nil {
...
func observeReceiveSignal(t *testing.T) {
	...
	go func() {
		...
		defer runtime.UnlockOSThread()
		thread <- unix.Gettid()
		event, file, err := startup.ReceiveEvent(control)
	...
	_, extra, e := startup.ReceiveEvent(control)
	if extra != nil {
		extra.Close()
