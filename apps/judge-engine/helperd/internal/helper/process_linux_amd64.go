...
func (p *isolatedProcess) prepareInitResources(group executionGroup) error {
...
func (p *isolatedProcess) startInit(r hostexec.Request, executable string) error {
	...
	cmd.ExtraFiles = []*os.File{
		launcher.InitControlFD - launcher.ExtraFilesBaseFD:  p.childControl.File,
		launcher.InitInputFD - launcher.ExtraFilesBaseFD:    p.dataR.File,
		launcher.InitLivenessFD - launcher.ExtraFilesBaseFD: p.lifeR.File,
	}
...
func (p *isolatedProcess) exchangeWithInit(r hostexec.Request, source io.Reader) {
	...
	go func() {
		...
		for i := 0; i < maxInitEvents; i++ {
			e, f, err := launcher.ReceiveEvent(p.control.File)
...
func (d workspaceDirectory) Open(name string) (*os.File, int64, error) {
	return launcher.OpenOutput(d.File.File, name)
}
