...
// runInit 在 namespace PID 1 内协调可信启动；主线程的挂载与降权状态必须连续，
// 不能返回服务循环，也不能在中途迁移到别的 OS 线程。
func runInit() {
	runtime.LockOSThread()
	control := os.NewFile(InitControlFD, "helper-control")
...
func (s *initSession) run() error {
	src := os.NewFile(InitInputFD, "stage-input")
	life := os.NewFile(InitLivenessFD, "helper-liveness")
	s.own(src)
...
func (s *initSession) reportExit() error {
	...
	var record [execFailureRecordBytes]byte
	n, readErr := io.ReadFull(s.execErrors, record[:])
	report.ExecFailed = n != 0 || readErr != io.EOF
	if n == len(record) && readErr == nil {
		report.ExecStage = record[execFailureStageOffset]
		report.ExecErrno = binary.LittleEndian.Uint32(record[execFailureErrnoOffset:])
	}
