...
适用于本仓库所有 Go 代码。跨语言的通用约定（单位、契约、零值陷阱、
什么进配置什么进请求、测试观念）见根目录 [`CLAUDE.md`](../../CLAUDE.md)，
本文只写 Go 特有的部分。
...
才有信息量」「给意图起个名字」——见 [`CLAUDE.md`](../../CLAUDE.md) §1.1，那里的
...
通用的测试观念见 `CLAUDE.md` §1.8，这里只写 Go 的写法：
