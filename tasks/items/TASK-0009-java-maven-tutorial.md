...
updated_at: "2026-08-20T11:10:43+08:00"
...
- JDK 25、Maven 和项目工作区的前置检查与本机版本纠正。
...
- [ ] 教程头部明确 M3 阶段、任务分支规则、依赖 M2/contracts v2 和本章产物。
- [ ] 所有命令都以仓库根目录为基准，并明确命令执行后的期望输出或文件树。
- [ ] 父 POM 与五个子 POM 给出可复制的完整文件，版本只有一个管理位置。
- [ ] Gateway 明确只使用 WebFlux；user/problem/submission/judging 明确只使用 Spring MVC。
- [ ] 教程提供最小 Java 类、配置和测试，并保留后续业务实现的 TODO 而不提前填答案。
- [ ] 验收覆盖 JDK 25、Maven Wrapper、聚合构建、单模块测试和五个健康检查。
- [ ] 常见坑覆盖本机 Java 22、模块路径、parent.relativePath、端口占用和 Web MVC/WebFlux 混装。
- [ ] `tutorial/README.md` 已加入 M3，并且 Markdown 代码围栏成对。
