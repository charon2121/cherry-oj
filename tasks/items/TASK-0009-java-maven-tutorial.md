...
updated_at: "2026-08-20T11:20:14+08:00"
...
- [x] 验收覆盖 JDK 25、Maven Wrapper、聚合构建、单模块测试和五个健康检查。
...
已完成 Java Maven 五服务初始化教程。教程覆盖本机 Java 22→25 的纠正、父子 POM、Maven Wrapper、
Gateway WebFlux 与四个 MVC 服务、最小启动类/配置/测试、依赖树边界、构建与健康检查，并明确推迟
数据库、Kafka、Redis、Security、业务 API 和 contracts DTO 实现。已核对 Boot 4.1.0/Cloud 2025.1.2
官方兼容性；102 个 Markdown 围栏、6 份完整 XML、5 份 YAML、本地链接与任务状态检查均通过。
`apps/server` 未改动，实际 Maven 构建与健康检查由用户按教程执行。
