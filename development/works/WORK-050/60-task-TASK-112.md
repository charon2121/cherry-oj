...
- [x] 空白四业务库/Redis/Kafka、五真实Java服务及原生Linux节点使用运行时凭据和独占资源，不依赖IDEA或旧ZIP。
- [x] 正常bootstrap/登录/CSRF、节点注册与环境选用、6对数据上传/部署、独立校准及发布通过；不直接写业务表。
- [x] 8类自定义覆盖stdout/stderr、CE/RE/普通SIGKILL、CPU/MLE/OLE及紧接空程序；CPU运行墙钟与HTTP总耗时分开取证。
- [x] 正式AC 6/6和WA走真实Kafka链，确认版本/环境/数据/校准关联，代码回看与编辑草稿保持正确。
- [x] 页面测试禁止route.fulfill/模拟后端；审计和请求ID来自本轮，报告不包含凭据/会话。
- [x] 最终无任务进程/cgroup/挂载/工作文件残留；全栈和独占卷退出清理，正式提交不自动重试掩盖失败。
...
- 2026-09-12：a69c893诊断已在Linux实跑，公开题目接口404且编辑器未创建。确认CI发布后遗漏PUBLIC管理操作；本批仅补正常公开与身份读回、失败反例。独立复核无阻塞，本地138项通过，按既有授权提交修复并运行真实CI，任务仍doing。
- 2026-09-12：状态变更：doing → done。原因：4b5df32完整CI34699419677的11job通过，业务15/15、内核63/63、原生10/10和全空清理报告复验通过；独立复核无阻断
