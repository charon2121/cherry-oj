...
WORK-053验收后继续TASK-112，先补浏览器失败取证：只导出固定阶段名、Playwright状态、固定测试文件名及行列号，最多16个错误位置；不导出错误正文、动态步骤标题、调用参数、DOM或网络载荷。Python驱动在清理前按白名单重新校验诊断，未知字段或超限拒绝，不影响原始失败与清理。新增登录/题目导航步骤仅区分启动失败阶段，不改变业务断言、次数或期限；发布沿TASK-112既有本批CI修正授权。

TASK-112 2026-09-12 诊断细化：WORK-057验收后，io仍在等待编辑器可见时失败。沿既有脱敏取证路线，允许额外导出固定editor对象：匹配数量(0..16封顶)、可见/加载中/加载失败布尔值、题目详情最后HTTP状态(0或100..599)。只输出这些计数/布尔/状态码，不输出DOM、URL、标题、错误正文、源码或响应正文；缺失时不伪造。Playwright失败后仅一次有界观察，保留原异常；reporter与Python导出入口均验证结构，未知字段/类型拒绝。所有原断言与期限不变，不对失败请求重试。

## TASK-113 汇总实施细化（2026-09-12）

新增sandbox-summary作为稳定必需检查，needs现有11个job并always执行，仅contents:read，在独立无特权VM下载本轮basic/kernel/native/business四份报告，逐一验证93项、清理和源码/harness身份。报告升级为v2并携带runAttempt，拒绝不同run或不同attempt拼接；原v1历史证据仍按原提交校验，不追改历史。产物名加run_id/run_attempt，避免全量重跑覆盖旧批次。汇总仅解析数据，固定白名单输出各job/套件状态和错误分类，不导出任意异常正文。

本地正反例执行同一汇总入口，覆盖前置失败/取消/跳过、错SHA/run/attempt/harness、缺失/重复case、能力不足、缺报告/证据、清理失败及文件链接。Linux先验证这批反例和完整结果，再执行实际Actions取消与至少两轮同SHA全量成功，其中一轮显式绕过包缓存读取（不清空公共缓存）；不能把建模的cancelled状态当作真实取消清理证据。汇总或下载失败仍须输出失败状态，不能用continue-on-error放行。
