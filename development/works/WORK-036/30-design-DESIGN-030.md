...
## 构图特征的重新识别（2026-09-04）

样板页交付后，用户判定"不符合预期"，并指出三处：**整体版式还是居中卡片、筛选区像后台表单、
列表有表头太像表格**；参照物明确为 **Linear 本身**。这触发了本文原定的重审条件，
因此在继续迁移其余页面之前修正构图识别。

**原识别漏掉的那一层**：本文此前把注意力放在"行内部怎么排"（列宽、密度、hairline、状态点），
这些都做对了；但没有识别"页面外壳怎么放"。结果是**一个正确的列表被装进了一个后台管理页面的
外框里**——居中 1200px 容器、圆角边框、列头行。三样都是数据表的语言，不是应用工作区的语言。

Linear 这类工程工具的列表页特征，逐条可判定：

| 特征 | 判据 | 我们此前 |
|---|---|---|
| 全宽铺满 | 列表左右边缘贴着主内容区边缘，没有居中留白 | 居中 1200px 容器，两侧大片空白 |
| 无卡片外框 | 列表不套圆角边框；边界由 hairline 和留白建立 | `rounded-lg border` 包住整个列表 |
| 无列头 | 直接就是行；列的含义由内容自明（mono 是标识、右对齐是度量） | 有 `标识/题目/标签/难度/模式/语言` 一整行列头 |
| 筛选是紧凑控件 | 一排小控件，不带竖排标签，不占整行 | 四个下拉，每个上方竖排一行标签 |
| 工具条与列表连成一体 | 工具条是列表的头部，不是浮在它上面的另一个盒子 | 工具条与列表之间有边框分隔 |

**列头是可选的，不是禁止的。** 判据是这个界面属于"应用工作区"还是"数据管理表"：
用户端题库、提交列表属于前者，默认无列头；管理端的批量数据表属于后者，可以有列头。
`DataList` 因此需要 `showHeader` 开关，默认关闭。

**全宽与外壳的边界。** 去掉卡片外框和居中容器属于本次范围。用户端是否也换成 Linear 式的
左侧固定侧栏（当前是顶部导航栏），影响每个页面与两个 Shell，**不在 WORK-036 范围内**，
留作独立工作项由用户决定。本次先把"列表页不再是居中卡片"做到，再评估外壳。
...
- **样板页被判"仍然不像"。** 这是本次最重要的重审条件：说明构图合同抓错了特征。
  正确反应是回到本文重新识别，而不是继续迁移其余页面把问题放大。
  **2026-09-04 已触发一次**，识别结果见上一节：漏掉的是"页面外壳"这一层，不是行内部的排布。
  若第二版仍被判不像，下一个怀疑对象是应用外壳（顶部导航 vs 左侧固定侧栏），
  而那属于独立工作项，不要在 WORK-036 内扩大。
