...
开发者在开发工具里构建或启动用户服务时，会立刻遇到「找不到共享日志组件」而失败，服务还没开始编译
就停住了。这个组件其实已经在项目里，问题不是缺东西，而是开发工具把用户服务当成了一个可以脱离后端
整体、单独构建的项目。本次提供一个不会选错入口的启动方式，并把正确的构建入口写清楚：开发者不必
往自己电脑里额外装一份组件，也不必靠删掉日志功能来绕开错误。
