...
flowchart TD
    ...
    class definition,design doing
    class memory skipped
```

| 阶段 | 状态 | 必需性 | 依据文档 | 说明 |
|---|---|---|---|---|
| 问题说明、复现与预期 | ▶ 进行中 | 必需 | ISSUE-004 `review` | 说清楚这件事要达成什么、边界在哪、怎样算完成 |
| 原因与修复方案 | ▶ 进行中 | 必需 | DESIGN-017 `review` | 确定技术方案、边界与取舍 |
| 修复任务 | · 未开始 | 必需 | TASK-029 `todo` | 拆成可独立完成并验证的任务，划定可读、可写与禁止范围 |
| 开发 | · 未开始 | 必需 | TASK-029 `todo` | 按任务实施，产出代码与测试 |
| 复核 | · 未开始 | 必需 | — | 独立复核实现是否符合定义与方案，边界有没有被越过 |
| 回归验证 | · 未开始 | 必需 | VERIFY-021 `draft` | 用可复现的证据确认要求逐条满足 |
