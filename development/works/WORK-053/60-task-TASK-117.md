...
- [x] 实际authenticate→MySQL→validate/exchange在纳秒固定时钟下旧红新绿，签发值不晚于配置期限。
- [x] 最小生产修改及原有过期/撤销/固定期限回归通过。
- [x] CI明确执行必需数据库测试且拒绝skip，保留失败与所有权清理事实。
- [x] Linux正常登录/改密/重新登录通过，接续TASK-112并诚实记录全部业务结果。
- [x] 独立复核与技术证据齐备；人工验收闸继续由用户签署。
...
- 2026-09-11：状态变更：doing → done。原因：AC001至004技术要求已由旧红新绿、正常改密和独立复核满足；后续浏览器失败留TASK112，最终人工验收待用户
...
- 2026-09-11：31b4019在Linux真实MySQL8/8零跳过，正常登录/改密/重登及部署校准发布成功，独立源码与证据复核通过。后续浏览器失败保留TASK-112，WORK-053技术完成待人工验收。
