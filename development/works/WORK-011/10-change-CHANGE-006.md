...
判题相关的两个程序在记录运行日志时，日志的格式和内容是在业务流程中间一项项拼出来的。这带来两个
问题：日志该记哪些内容散落在业务代码各处，没有一个地方能一次看全；以后要调整日志内容，就得改动
本来与日志无关的业务代码。本次把日志的组织方式收拢到一处，业务流程只负责说「这件事完成了」，
不再关心日志长什么样。记录的内容、级别和敏感信息边界保持不变。
