...
def download(lock, output, resume=False):
    ...
    def fetch(package):
        ...
        h = hashlib.sha256()
        total = 0
        with urllib.request.urlopen(url, timeout=30) as response, target.open('xb') as file:
...
if __name__ == '__main__':
    ...
    download(args.lock, args.output, args.resume)
