...
> 这里是**本项目自己踩出来的**跨语言约定，优先级高于 [`general.md`](./general.md) 的通用指令
> （见那里的 Priority Rules：项目已有约定排第 2，通用规范排第 4）。
> 语言和框架专属规范见 `languages/` 与 `frameworks/`。
>
> 本文的章节号被其他规范文档直接引用（`§1.4`、`§1.9` 等），**调整章节时要同步改引用**。
...
> `http.Client` 零值永不超时）见 [`languages/go.md`](./languages/go.md) §6。
...
- **已知技术债必须有编号。** 先在所属 `development/works/WORK-xxx/` 建立 `60-task-TASK-xxx.md`，
  代码里的锚点写成 `TODO(TASK-001): ...`。没有编号的 `TODO` 等于没人认领，也无法被
  `scripts/work` 追踪到——它会在下一次重构里被当成噪声删掉，问题却还在。
- **教程里的 `// TODO(你来写)` 是内容，不是待办。** `tutorial/` 的代码是给人照着写的，
  留白本身就是教学设计。不要「顺手补全」，那会把练习变成抄写。
